...
class MetaApiService {
  ...
  async getSymbolPrice(symbol) {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api'}/prices/${symbol}`)
  ...
  async getAllPrices(symbolList) {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001/api'}/prices/batch`, {
        method: 'POST',
